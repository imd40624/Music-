...
async def help_moderations(ctx):
   ...
   embed.add_field(name = 'd?announce(Admin permission required)', value="Use it liked ``?announce #channel <text>``",inline = False)
   embed.add_field(name = 'd?prefix(Admin permission required)', value="Use it liked ``d?prefix ?``",inline = False)
   embed.add_field(name = 'Note:', value="***More commands being added soon!***")
   embed.set_footer(text="Requested by: " + author.name)
   await bot.say(embed=embed)
